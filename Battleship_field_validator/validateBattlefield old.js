export function validateBattlefield(field) {
    ...
    let IsBattlefieldValid = true;
    for (let i = 0; i < 10; i++) {

        for (let j = 0; j < 10; j++) {
            if (field[i][j] == 1) {
                validateSurroundingCordinates(i, j);
                if (!IsBattlefieldValid) { return IsBattlefieldValid; }
            }
        }
    }
    ...
    function validateSurroundingCordinates(x, y) {
        ...
        if (!(
            ...
            matrixEqual(surroundingCordinates, [
                ...
                [0, 1, 1],
                [0, 0, 0]])
        )) { IsBattlefieldValid = false; return surroundingCordinates; }
...
if(ships.battleship!=1||ships.cruiser!=2||ships.destroyer!=3||ships.submarine!=4){IsBattlefieldValid = false;}

    return IsBattlefieldValid;
}